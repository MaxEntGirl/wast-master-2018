...
<!--    <xsl:template match="*" mode="version">
    ...
    <xsl:template match="tei:pb[not(@rend='shyphen')and not(preceding-sibling::*[1][matches(@rend,'shyphen','i')]) and ancestor::*:s]" priority="0.9" mode="text"><xsl:text> </xsl:text></xsl:template>
...
<xsl:text>
      ...
      <xsl:variable name="n-attribute" select="@n"/>
        <xsl:analyze-string select="$phase1" regex="^([^\n]+)$" flags="m">
          <xsl:matching-substring>
              <xsl:sequence select="concat($n-attribute,';',normalize-space(regex-group(1)))"/>
          </xsl:matching-substring>
    ...
    <xsl:template match="tei:s" mode="text" >
        <!-- overriding sentence to add space after next match(wab2cis_text.xsl) Which will be normalized to slice last space in ab.-->
        <xsl:next-match/>
